...
export default function ProfileWizard({ profile, onChangeProfile, riskAnalysis, onTriggerEvaluate, isEvaluating }) {
  ...
  const getCapacityBadge = (capacity) => {
    switch (capacity) {
      ...
      case 'moderate':
        return <span className="px-2.5 py-0.5 rounded-full text-sm font-bold bg-yellow-50 text-yellow-800 border border-amber-500/40">MODERATE BUFFER</span>;
      case 'low':
      default:
        return <span className="px-2.5 py-0.5 rounded-full text-sm font-bold bg-red-50 text-red-700 border border-red-200">LOW BUFFER</span>;
    }
  };

  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm rounded-2xl p-5 md:p-6 shadow-xl space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 border-b border-gray-200">
        <div className="flex items-center gap-2.5">
          <div className="p-2 rounded-lg bg-indigo-50 text-indigo-600 border border-indigo-600/20">
            <Sliders className="h-5 w-5" />
          </div>
          <div>
            <h2 className="text-base font-bold text-gray-900 tracking-tight">Your Financial Situation & Constraints</h2>
            <p className="text-sm text-gray-600">Real-time profile parameters powering suitability scoring</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-sm text-gray-600 font-medium">Profile Name:</span>
          <span className="text-sm font-bold text-indigo-600 bg-gray-100 px-2.5 py-1 rounded-md border border-gray-200">
        ...
        <div className={`p-4 rounded-xl border transition-all ${
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3 shrink-0">
              <div className="bg-gray-50/80 px-3 py-2 rounded-lg border border-gray-200 text-center min-w-[100px]">
                <span className="text-sm uppercase font-bold text-gray-600 block">Surplus / mo</span>
                <span className="text-sm font-extrabold text-indigo-600">
                  ₹{riskAnalysis.monthly_surplus.toLocaleString('en-IN')}
                </span>
                <span className="text-sm text-gray-600 block">({riskAnalysis.surplus_ratio_pct}%)</span>
              </div>
              <div className="bg-gray-50/80 px-3 py-2 rounded-lg border border-gray-200 text-center min-w-[95px]">
                <span className="text-sm uppercase font-bold text-gray-600 block">Runway</span>
                <span className={`text-sm font-extrabold ${riskAnalysis.emergency_runway_months < 3 ? 'text-red-600' : 'text-indigo-600'}`}>
                  {riskAnalysis.emergency_runway_months} Mo
                </span>
                <span className="text-sm text-gray-600 block">buffer</span>
              </div>
              <div className="bg-gray-50/80 px-3 py-2 rounded-lg border border-gray-200 text-center min-w-[85px]">
                <span className="text-sm uppercase font-bold text-gray-600 block">DTI Ratio</span>
                <span className={`text-sm font-extrabold ${riskAnalysis.debt_to_income_pct > 40 ? 'text-red-600' : 'text-gray-800'}`}>
                  {riskAnalysis.debt_to_income_pct}%
                </span>
                <span className="text-sm text-gray-600 block">debt load</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="space-y-1.5 bg-white p-3.5 rounded-xl border border-gray-200">
          <label className="text-sm font-semibold text-gray-700 flex items-center justify-between">
            ...
            <span className="text-indigo-600 font-mono font-bold">₹{Number(profile.monthly_income).toLocaleString('en-IN')}</span>
          </label>
          <input
            type="range"
            min="10000"
        ...
        <div className="space-y-1.5 bg-white p-3.5 rounded-xl border border-gray-200">
          <label className="text-sm font-semibold text-gray-700 flex items-center justify-between">
            ...
            <span className="text-yellow-700 font-mono font-bold">₹{Number(profile.monthly_expenses).toLocaleString('en-IN')}</span>
          </label>
          <input
            type="range"
            min="5000"
        ...
        <div className="space-y-1.5 bg-white p-3.5 rounded-xl border border-gray-200">
          <label className="text-sm font-semibold text-gray-700 flex items-center justify-between">
            ...
            <span className="text-red-600 font-mono font-bold">₹{Number(profile.existing_debt_emi).toLocaleString('en-IN')}</span>
          </label>
          <input
            type="range"
            min="0"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="space-y-1.5 bg-white p-3.5 rounded-xl border border-gray-200">
          <label className="text-sm font-semibold text-gray-700 flex items-center justify-between">
            ...
            <span className="text-gray-900 font-mono font-bold">₹{Number(profile.target_amount).toLocaleString('en-IN')}</span>
          </label>
          <input
            type="range"
            min="10000"
        ...
        <div className="space-y-1.5 bg-white p-3.5 rounded-xl border border-gray-200">
          <label className="text-sm font-semibold text-gray-700 flex items-center justify-between">
            ...
            <span className="text-sm font-bold uppercase text-indigo-600">{profile.risk_tolerance}</span>
          </label>
        ...
        <div className="space-y-1.5 bg-white p-3.5 rounded-xl border border-gray-200">
          <label className="text-sm font-semibold text-gray-700 flex items-center justify-between">
            ...
            <span className="text-sm font-bold capitalize text-indigo-600">
